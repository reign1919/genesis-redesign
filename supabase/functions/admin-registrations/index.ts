...
import { decryptPassword, encryptPassword, generateSchoolPassword, schoolAuthEmail } from '../_shared/credentials.ts';
...
const REGISTRATION_FIELDS = 'id,school_name,teacher_whatsapp,school_code,status,created_at,updated_at';
...
function safeAuthError(error: unknown): string {
  if (!error || typeof error !== 'object') return 'Supabase Auth rejected user creation.';
  const candidate = error as { code?: unknown; message?: unknown; status?: unknown };
  const code = typeof candidate.code === 'string' && /^[a-z0-9_-]{1,80}$/iu.test(candidate.code)
    ? candidate.code
    : null;
  const message = typeof candidate.message === 'string'
    ? candidate.message
      .replace(/[\r\n]+/gu, ' ')
      .replace(/gen-[0-9]{4}@[a-z0-9.-]+/giu, '[internal school alias]')
      .slice(0, 180)
    : null;
  return [code, message].filter(Boolean).join(': ') || `Supabase Auth returned status ${candidate.status || 'unknown'}.`;
}
...
export async function handleAdminRegistrations(
  ...
  try {
    ...
    if (!adminClient) return jsonResponse(stableBody(false, 'SERVICE_UNAVAILABLE', 'Administration is temporarily unavailable.', requestId), 503, cors.headers);
    ...
    if (!await checkAdmin(adminClient, user.id)) {
      ...
      return jsonResponse(stableBody(false, 'FORBIDDEN', 'You do not have access to this area.', requestId), 403, cors.headers);
    }

    const encryptionKey = env('SCHOOL_CREDENTIAL_ENCRYPTION_KEY');

    if (request.method === 'GET') {
      if (!encryptionKey) return jsonResponse(stableBody(false, 'SERVICE_UNAVAILABLE', 'Credential configuration is unavailable.', requestId), 503, cors.headers);
      const { data, error } = await adminClient
        .from('registrations')
        .select(`${REGISTRATION_FIELDS},school_credentials(password_ciphertext,password_iv)`)
        .order('created_at', { ascending: false })
        .limit(500);
      if (error) return jsonResponse(stableBody(false, 'SERVICE_UNAVAILABLE', 'Registrations could not be loaded.', requestId), 503, cors.headers);

      const registrations = await Promise.all((data || []).map(async (row: Record<string, unknown>) => {
        ...
        if (related && typeof related === 'object') {
          ...
          if (typeof credential.password_ciphertext === 'string' && typeof credential.password_iv === 'string') {
            password = await decryptPassword(credential.password_ciphertext, credential.password_iv, encryptionKey);
          }
        }
      ...
      return jsonResponse(stableBody(true, 'REGISTRATIONS_LISTED', 'Registrations loaded.', requestId, { registrations }), 200, cors.headers);
    }

    const payload = await readJsonObject(request, 2048);
    if (!payload) return jsonResponse(stableBody(false, 'INVALID_PAYLOAD', 'Check the requested status change.', requestId), 400, cors.headers);
    const validated = validateAdminTransition(payload);
    if (!validated.ok) return jsonResponse(stableBody(false, validated.code, 'Check the requested status change.', requestId), 400, cors.headers);
    ...
    if (validated.value.status === 'rejected') {
      const { data, error } = await adminClient
        ...
        .maybeSingle();
      if (error) return jsonResponse(stableBody(false, 'SERVICE_UNAVAILABLE', 'The registration could not be updated.', requestId), 503, cors.headers);
      if (!data) return jsonResponse(stableBody(false, 'INVALID_TRANSITION', 'This registration is no longer pending.', requestId), 409, cors.headers);
      return jsonResponse(stableBody(true, 'REGISTRATION_UPDATED', 'Registration rejected.', requestId, { registration: data }), 200, cors.headers);
    }
    ...
    if (reserveError || typeof reservedCode !== 'string') {
      return jsonResponse(stableBody(false, 'SERVICE_UNAVAILABLE', 'Credentials could not be generated.', requestId), 503, cors.headers);
    }
    if (reservedCode === 'invalid_transition' || reservedCode === 'not_found') {
      return jsonResponse(stableBody(false, 'INVALID_TRANSITION', 'This registration is no longer pending.', requestId), 409, cors.headers);
    }
    if (reservedCode === 'code_exhausted') {
      return jsonResponse(stableBody(false, 'CODE_EXHAUSTED', 'No school codes remain. Contact the system administrator.', requestId), 503, cors.headers);
    }

    let createdAuthUserId: string | null = null;
    let provisioningStage = 'load_staging';
    let provisioningDetail = '';
    try {
      ...
      const password = await decryptPassword(staged.password_ciphertext, staged.password_iv, encryptionKey);
      let authUserId = staged.auth_user_id as string | null;
      if (!authUserId) {
        provisioningStage = 'create_auth_user';
        const email = schoolAuthEmail(reservedCode, env('SCHOOL_AUTH_EMAIL_DOMAIN') || 'schools.genesis.invalid');
        ...
        if (created.error || !created.data.user) {
          provisioningDetail = safeAuthError(created.error);
          provisioningStage = 'recover_auth_user';
          const existing = await findAuthUserByEmail(adminClient, email);
          if (!existing || existing.app_metadata?.school_registration_id !== validated.value.registrationId) {
        ...
        const passwordUpdate = await adminClient.auth.admin.updateUserById(authUserId, { password });
      ...
      if (finalizeError || finalizeCode !== 'approved') throw finalizeError || new Error('approval not finalized');

      provisioningStage = 'reload_registration';
      const { data: registration, error: registrationError } = await adminClient
        .from('registrations').select(REGISTRATION_FIELDS)
        .eq('id', validated.value.registrationId).single();
      if (registrationError) throw registrationError;
      const withCredentials = { ...registration, password };
      const message = whatsappMessage(withCredentials, env('PUBLIC_PORTAL_URL') || 'https://genesis.example/#/portal');
      safeLog('info', 'admin_transition_completed', requestId, { status: 'approved', transition: 'approved' });
      return jsonResponse(stableBody(true, 'REGISTRATION_UPDATED', 'Registration approved and credentials generated.', requestId, {
        registration: withCredentials,
        whatsappMessage: message,
      }), 200, cors.headers);
    } catch {
      if (createdAuthUserId) await adminClient.auth.admin.deleteUser(createdAuthUserId);
      await adminClient.rpc('reset_registration_approval', { p_registration_id: validated.value.registrationId });
      safeLog('error', 'admin_credential_provisioning_failed', requestId, {
        code: 'PROVISIONING_FAILED',
        stage: provisioningStage,
      });
      return jsonResponse(stableBody(false, 'PROVISIONING_FAILED', 'Credentials could not be generated. Try again.', requestId, {
        stage: provisioningStage,
        ...(provisioningDetail ? { detail: provisioningDetail } : {}),
      }), 503, cors.headers);
    }
  } catch {
    safeLog('error', 'admin_unhandled_failure', requestId, { code: 'UNEXPECTED_ERROR' });
    return jsonResponse(stableBody(false, 'SERVICE_UNAVAILABLE', 'Administration is temporarily unavailable.', requestId), 503, cors.headers);
  }
}
